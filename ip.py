...
class IP:
	...
	def invasion(self):
			...
			for i in range(Nrow):
				...
				if sumrow < Ncol and i != (self.Nrow-1):
					for j in range(Ncol):
						...
						if boolean and pressurelattice[i+1,j]<tmp:
							tmp  = pressurelattice[i+1,j]; ytmp = i+1; xtmp = j


			invasionlattice[ytmp,xtmp] = 0
			pressure.append(tmp)
			saturation.append(1 -np.sum(invasionlattice[:,:])/float((Ncol*(Nrow-1))))
		...
		np.savetxt(self.pathsimulation + "saturation"+str(Ncol)+"_"+str(Nrow)+".txt",[saturation])

		#plotting figures
		self.plotting(pressurelattice,invasionlattice,pressure,number_of_clusters,saturation)
	...
	def plotting(self,pressurelattice,invasionlattice,pressure,number_of_clusters,saturaiton):
		...
		plt.figure(5)
		plt.plot(saturaiton,pressure)
		plt.xlabel('saturation')
		plt.ylabel('Pressure')
		plt.title('Pressure saturation curve')
